...
def plot_atlas_based_surface(t_values, title):
    ...
    plotting.plot_surf_stat_map(
        ...
        colorbar=True, cmap='RdBu_r', threshold=0.01, darkness=None,
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, f"Surface_{title}.png"), dpi=300, bbox_inches='tight')
...
def run_pipeline():
    ...
    sns.boxplot(x='Group_MIND', y='Global_Strength', data=df_final, palette='vlag', hue='Group_MIND', legend=False)
    sns.stripplot(x='Group_MIND', y='Global_Strength', data=df_final, color=".3", alpha=0.3)
    ...
    sns.heatmap(pd.DataFrame(strength_mat).corr(), cmap='coolwarm', center=0, xticklabels=False, yticklabels=False)
    plt.title("Connectome Correlation Matrix (Baseline)")
    plt.savefig(os.path.join(OUTPUT_DIR, "Connectome_Matrix.png"))
