...
def run_nodal_analysis():
    ...
    plt.bar(range(1, 69), -np.log10(anova_fdr), color='teal')
    plt.axhline(-np.log10(0.05), color='red', linestyle='--', label='p < 0.05 (FDR)')
    ...
    sns.boxplot(x='Group_MIND', y='best_node', data=meta, palette='vlag', hue='Group_MIND', legend=False)
    sns.stripplot(x='Group_MIND', y='best_node', data=meta, color="black", alpha=0.3)
